...
- ~~**Sem rate limiting em nenhum endpoint público**~~ — resolvido em 2026-09-17 (`RateLimitFilter`, P2-12), ver `decisions.md`. Limite por IP (Redis, fixed-window) nas 4 rotas que disparam LLM: `GET /api/stocks/{ticker}/analysis`, `POST /api/stocks/{ticker}/analysis/refresh`, `GET /api/compare`, `POST /api/simulate`. **Limite conhecido, aceito por ora**: falha do Redis é fail-open (loga warn, libera a requisição) — um ataque coordenado que também sobrecarregue o Redis remove a proteção justamente quando mais importaria; fallback fail-closed local (contador em memória de curto prazo) não implementado.
...
- **Nenhum teste/alerta exercita o fallback Groq contra a API real** — o único jeito de descobrir que `qwen/qwen3-32b` tinha sido descomissionado pela Groq (2026-08-10, ver `decisions.md`) foi um smoke test manual de `tools/promptfoo`. Os testes existentes mockam `ChatModel`, nunca fazem round-trip HTTP real contra Gemini/Groq — um modelo sumir do catálogo do provedor não gera nenhum sinal até o Gemini falhar de verdade em produção e o fallback também falhar. Não implementado ainda: alerta/healthcheck periódico que valide os dois `modelName` contra `GET /v1/models` de cada provedor, ou um teste (não em CI, já que não há CI) que rode isso manualmente antes de deploy.
