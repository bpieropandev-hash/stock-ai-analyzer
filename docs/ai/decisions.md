...
## Fallback Groq trocado de `qwen/qwen3-32b` para `qwen/qwen3.6-27b` (2026-08-10)

**Decisão**: `EmbeddingStoreConfig#groqChatModel` e o literal `modelUsed` em `StockAnalysisService` atualizados de `qwen/qwen3-32b` para `qwen/qwen3.6-27b`.
**Motivo**: achado incidental durante o spike de `tools/promptfoo` (regression testing de prompt, não relacionado a troca de modelo). O smoke test do `llm-rubric` contra o provider Groq retornou `404 model_not_found`. Confirmado fora do promptfoo, direto contra a API real da Groq (`GET /v1/models` com a chave real do projeto): `qwen/qwen3-32b` não existe mais no catálogo, `qwen/qwen3.6-27b` é o modelo disponível hoje.
**Impacto real, não hipotético**: o fallback Gemini→Groq (`StockAnalysisService`, linha ~179-189) estava morto — se o Gemini falhasse por qualquer motivo, a tentativa de fallback pro Groq também falhava (mesmo erro 404), e a exceção final (`"Análise temporariamente indisponível"`) chegava ao usuário sem nunca ter havido fallback de verdade. Não é o cenário "funcionalidade opcional de IA degrada" do `invariants.md` — é o path de scoring principal perdendo sua única rede de segurança contra outage do provedor primário.
**Por que não foi pego antes**: não há teste automatizado que exercite o path de fallback contra a API real da Groq (os testes existentes mockam `ChatModel`, não fazem round-trip HTTP) — nem alerta de erro-rate por modelo. `qwen3-32b` foi provavelmente descomissionado pela Groq em algum momento entre a implementação original e 2026-08-10, sem que nada no sistema notasse.
**Risco documentado, não resolvido aqui**: esta classe de risco ("modelo hospedado por nome pode sumir sem aviso") já estava registrada em `anti-patterns.md`/`PROJECT_DOCUMENTATION.md` como risco de comparabilidade de score — mas o risco real que se materializou foi mais severo (indisponibilidade total do fallback, não só quebra de comparabilidade histórica). Nenhum monitoramento/alerta foi adicionado para detectar a próxima vez que isso acontecer — ver `anti-patterns.md`.
**Verificado**: `mvn clean compile` limpo, suíte completa sem regressão (troca é só um literal de string), e round-trip real contra a Groq confirmado via `tools/promptfoo` (chamada real ao `qwen/qwen3.6-27b` retornou 200 com JSON válido).
**Ver também**: `ai.md`, `anti-patterns.md`, `tools/promptfoo/README.md`.

## `reasoningEffort("none")` portado pro `groqChatModel` de produção (2026-09-17)

**Decisão**: `EmbeddingStoreConfig#groqChatModel` ganhou `.reasoningEffort("none")` no builder do `OpenAiChatModel` (LangChain4j 1.11.0 já expõe esse campo nativamente — sem precisar de `customParameters`/raw body override).
**Motivo**: `qwen/qwen3.6-27b` é um modelo de raciocínio — sem essa flag, ele gasta parte do budget de `maxTokens` (4096) num bloco `<think>` interno antes de emitir o JSON. Em picos de resposta mais longa isso estourava o budget (`400 json_validate_failed`, `failed_generation` vazio). Validado antes desta mudança via `tools/promptfoo` (sessão de 2026-08-11, ver memória `project-groq-fallback-model`): PETR4+VALE3 com `reasoning_effort=none` — 0 erros, `completion_tokens` 682/723; sem a flag, era 10/10 falhas no run anterior de 10 tickers com `maxTokens=4096`.
**Verificado**: `mvn clean compile` e suíte completa 33/33 limpos (Postgres/Redis reais via podman). **Não verificado nesta sessão com round-trip real contra a Groq** (sem chave/orçamento de LLM disparado agora) — a validação real do parâmetro já tinha sido feita separadamente via `tools/promptfoo` antes de portar; o que esta mudança adiciona é só o mesmo parâmetro no código de produção, mecanicamente equivalente ao teste já validado.
**Achado correlato, sem ação necessária**: o "sem retry/backoff no path Groq" registrado como risco pela sessão de 2026-08-11 estava incorreto — `OpenAiChatModel.doChat` (LangChain4j core) já envolve toda chamada em `RetryUtils.withRetryMappingExceptions` (`maxRetries=2` default, backoff exponencial com jitter, base 500ms); `429` é mapeado pra `RateLimitException`, que é retriável (não é `NonRetriableException`). Path Groq de produção já tem essa proteção nativa, sem código adicional.
**Ver também**: `ai.md`.

## Orquestração multi-agente formal ("IA comandando várias IAs") — avaliada e não adotada por ora (2026-09-17)

**Decisão**: manter o padrão atual (6 subagentes especialistas em `.claude/agents/`, acionados pontualmente quando a mudança cruza área — ver CLAUDE.md § "Especialistas disponíveis") em vez de criar uma camada de orquestração permanente tipo "IA gerente + equipe de IAs".
**Motivo**: Bernardo propôs modelo tipo empresa (uma IA orquestrando várias, cada uma como um "dev"). Já existe uma versão disso, só que sob demanda — o thread principal aciona os especialistas certos quando a tarefa cruza domínio (ex.: indicador financeiro novo aciona `financial-analyst` **e** `ai-engineer` em paralelo). Orquestração permanente adicionaria: custo (cada subagente é uma chamada de LLM própria, multiplica com volume de tarefas), fragmentação de contexto (um agente não vê a decisão do outro, só o que o thread principal repassar) e ainda exigiria alguém lendo/sintetizando os vereditos — não existe gate automático que aprove sozinho.
**Escala do projeto**: 1 dev, contexto compartilhado num único thread principal — o ganho de uma "empresa de IAs" formal aparece em times/tarefas grandes e verdadeiramente paralelizáveis, não no tamanho atual. Overhead de coordenação constante não compensa aqui.
**Não descartado, só não priorizado**: se o projeto crescer (mais devs, mais tarefas simultâneas de fato independentes), reavaliar. Nada foi implementado — é registro de decisão, não mudança de código.
**Ver também**: CLAUDE.md § "Especialistas disponíveis".

## Rate limiting por IP nos endpoints de análise (P2-12, resolvido em 2026-09-17)

**Decisão**: `RateLimitFilter` (`com.stockai.ratelimit`, novo pacote) — `OncePerRequestFilter` registrado em `SecurityConfig` antes do `JwtAuthFilter`, limita por IP as 4 rotas que disparam LLM: `GET /api/stocks/{ticker}/analysis`, `POST /api/stocks/{ticker}/analysis/refresh`, `GET /api/compare`, `POST /api/simulate`. Contador fixed-window em Redis (`StringRedisTemplate.increment` + TTL), sem dependência nova (bucket4j/resilience4j avaliados e descartados — Redis já é infra existente, contador simples resolve sem indireção). Limite configurável (`app.rate-limit.requests-per-minute`, default 20/min) e desligável (`app.rate-limit.enabled`).
**Motivo**: gap documentado em `anti-patterns.md` desde a auditoria de 2026-08-06 — endpoints públicos sem limite, cada miss de cache dispara custo real de LLM. Priorizado antes da expansão de universo de tickers pra B3 completa (ver `roadmap.md` § "Sequenciamento decidido") porque mais tickers expostos amplifica o risco de custo sem teto.
**Revisão de segurança**: `security-reviewer` (persona injetada em subagente, já que o tipo de agente não está registrado neste harness) aprovou com ressalva — achado bloqueante corrigido antes do commit: resolução de IP não confiava mais cegamente em `X-Forwarded-For` (era forjável por qualquer cliente direto, permitindo bypass total trocando de "IP" a cada requisição). Fix: `X-Forwarded-For` só é usado quando `request.getRemoteAddr()` está na allowlist `app.rate-limit.trusted-proxy-ips` (vazia por padrão = sempre `remoteAddr`, header ignorado).
**Limite aceito, não resolvido aqui**: falha do Redis é fail-open (loga warn, libera a requisição) — ver ressalva não-bloqueante do `security-reviewer` e nota em `anti-patterns.md`. Um ataque que também sobrecarregue o Redis remove a proteção. Fallback fail-closed local (contador em memória, curto prazo) fica como melhoria futura, não implementado agora.
**Fora do escopo por decisão**: `/api/alerts/**` (citado no gap original) não entra na lista — é leitura de alertas já persistidos, sem chamada LLM nova.
**Verificado**: `mvn clean compile` e suíte completa (Postgres/Redis reais via podman) verdes com JDK 26; bind de `Set<String>` vazio (`trusted-proxy-ips` sem valor) confirmado não quebra o contexto Spring.
**Ver também**: `anti-patterns.md`, `roadmap.md`.
